...
const Weight = (() => {
  ...
  function saveSectionOrder(order) {
    sectionOrder = order;
    Store.savePrefs(App.getUser(), { weight_section_order: order });
  }
  ...
  async function init(prefs) {
    prefs = prefs || {};
    if (prefs.weight_section_order) sectionOrder = prefs.weight_section_order;

    // Load weight config from Firestore
    const config = await Store.getConfig();
    if (config.weight_height_cm) {
      HEIGHT_CM = config.weight_height_cm;
      HEIGHT_M = HEIGHT_CM / 100;
    }
    if (config.weight_milestones) {
      MILESTONES = config.weight_milestones;
    }
