...
const Travel = (() => {
  ...
  async function init(user, prefs) {
    currentUser = user;
    prefs = prefs || {};

    // Load location config from Firestore
    const config = await Store.getConfig();
    locations = config.travel_locations || [
      { key: 'location_a', label: 'Location A' },
      { key: 'location_b', label: 'Location B' }
    ];

    // Restore saved direction from prefs (fall back to first configured location)
    currentDirection = prefs.travel_direction || locations[0].key;

    // Build direction selector buttons
    const selector = document.getElementById('direction-selector');
    selector.innerHTML = locations.map(loc =>
      `<button class="direction-btn" data-direction="${loc.key}">Leaving ${escapeHtml(loc.label)}</button>`
    ).join('');

    // Populate applies-to dropdown in travel modal
    const appliesTo = document.getElementById('travel-modal-applies-to');
    locations.forEach(loc => {
      const opt = document.createElement('option');
      opt.value = loc.key;
      opt.textContent = loc.label + ' only';
      appliesTo.appendChild(opt);
    });
  ...
  function setDirection(dir) {
    ...
    Store.savePrefs(currentUser, { travel_direction: dir });
    updateDirectionUI();
    render();
  }
  ...
  function closeTravelTaskModal() {
    ...
    const newTitle = document.getElementById('travel-modal-title').textContent.trim().slice(0, 500);
