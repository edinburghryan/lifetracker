...
A minimalist shared task management web app for two users (RC and LC).
...
| Hosting     | GitHub Pages (`<username>.github.io/lifetracker`)    |
| Database    | Firebase Firestore (Spark / free plan)                 |
| Auth        | Firebase Google Sign-In + user picker (RC / LC)        |
...
### 2.1 Google Sign-In + Identity
- Users authenticate via Firebase Google Sign-In (popup, with redirect fallback).
- After sign-in, a user picker (RC / LC) sets the active identity for task attribution.
- Session (identity) persisted in localStorage so users don't re-pick on every visit.
- Both users have identical permissions — the picker exists for attribution, not access control.
...
- Read/write requires `request.auth != null` (Firebase Google Sign-In).
- Optionally restrict to specific Google account UIDs for tighter access control.
